...
- [ ] ❌ **绝不在业务 UI / 新增组件里手写任何"视觉值"**——颜色、`:focus`/`:hover` 态、背景、边框、圆角、阴影、字重，一律走 `.ag-*` 现成类或 `var(--*)` token。
- [ ] ❌ **绝不自己写 `:focus` / `:hover` 规则**（最高频漏网点）。输入框/按钮/链接的交互态已内置在 `.ag-*` 类里，手写一套（尤其蓝色 focus）= 直接违规。
- [ ] ❌ **任何输入框必须用 `.ag-input`**，不许另起一套 `border`/`box-shadow`/`background` 的 focus 样式。
- [ ] **"想要的视觉效果 skill 里一定有"**：先找 `.ag-*` 类 / `var(--*)` token；确实找不到 → **停下来问，不许自己造值**。
- [ ] ✅ **唯一允许手写的是「布局」**：`display`/`flex`/`grid`/`gap`/`width` 等排布尺寸可手写（间距优先用 `var(--space-*)`）。组件的"长相"不许碰，组件的"排布"才归你管。
- [ ] ✅ `component-examples.html` 中的 `.ag-demo-*` 只服务组件画廊展示，**不要复制到业务代码**。
- [ ] ❌ 业务页面没有直接使用复杂结构类：`.ag-select` / `.ag-menu` / `.ag-modal` / `.ag-tabs` 必须来自 UI 组件封装或完整结构。
- [ ] ❌ 状态组件没有 `style={{ color: ... }}`；状态必须用 `Badge variant` / `StatusBadge status`。
...
- [ ] Select/Dropdown/Modal/Tabs 使用 React UI Kit 或项目基础组件，不在业务页面自拼半截结构。
- [ ] 大项目改造优先改 `components/ui/*`；业务页面没有大面积散落 `.ag-*` 和状态色工具类。

## 快速 grep 自查（可选）

对生成的业务组件 CSS 跑（不要扫 `tokens.css` / 官方 `components.css` / `.ag-demo-*` 画廊样式）：
```bash
...
# 手写 focus/hover 规则（应几乎为零——交互态都在 .ag-* 里）
...
```
有命中 → 逐个核对是否违规。`:focus`/`:hover` 命中尤其要警惕：除非是 token 化的 `.ag-*` 类内部，否则就是手写违规。
